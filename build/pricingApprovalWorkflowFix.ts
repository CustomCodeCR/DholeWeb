...
function replaceOne(source: string, anchor: string, replacement: string, label: string) {
  ...
  if (occurrences !== 1) {
    throw new Error(`[pricing-approval-workflow-fix] Expected one ${label} anchor, found ${occurrences}.`)
  }
...
function patchWizard(source: string) {
  ...
  code = replaceOne(
    ...
    "async function downloadCurrentQuote() {\n  if (!editingRate.value || downloadingQuote.value) return\n  if (!canDownloadCurrentQuote.value) {\n    toastStore.warning(\n      'Aprobación requerida',\n      'La cotización PDF estará disponible únicamente después de aprobar la tarifa.',\n    )\n    return\n  }\n  try {",
    'wizard pdf guard',
  )
  ...
  code = replaceOne(
    ...
    "            <p class=\"mt-3 text-xs font-semibold text-[var(--dh-text-muted)]\">\n              <template v-if=\"editingRate.status === 'PendingApproval'\">\n                Esta tarifa tiene un margen de {{ Number(editingRate.marginPercentage || 0).toFixed(2) }}% y debe ser aprobada antes de abrirse, enviarse o descargar el PDF.\n              </template>\n              <template v-else-if=\"editingRate.status === 'ApprovedByManagement'\">\n                La aprobación de gerencia ya fue registrada. Ponga la tarifa en Abierta para continuar el flujo comercial.\n              </template>\n              <template v-else>\n                Una tarifa Abierta puede marcarse Enviada. Después de Enviada puede registrarse como Aceptada o Rechazada.\n              </template>\n            </p>\n            <p v-if=\"editingRate.status === 'PendingApproval' && !canApproveLowMargin\" class=\"mt-2 text-xs font-black text-amber-600\">\n              Se requiere el permiso pricing.rate.approve-low-margin para aprobar esta tarifa.\n            </p>",
    'wizard approval help',
  )
...
export function pricingApprovalWorkflowFix(): Plugin {
  return {
