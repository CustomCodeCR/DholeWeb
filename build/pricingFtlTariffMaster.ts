...
function patchWizard(source: string) {
  ...
  code = replaceRequired(
    code,
    canNextAnchor,
    `const canNext = computed(() => {\n  if (step.value === 3 && shipmentModeForApi.value === 'Ltl' && lclDimensionalCbm.value <= 0) return false\n`,
    'LTL cargo validation',
  )

  const step3Index = code.indexOf(`<div v-else-if="step === 3" class="space-y-6">`)
  if (step3Index < 0) throw new Error('[pricingFtlTariffMaster] step 3 anchor not found.')
  const step3RouteCard = `          <div class="crystal-soft space-y-5 p-4 md:p-5">`
  const step3RouteIndex = code.indexOf(step3RouteCard, step3Index)
  if (step3RouteIndex < 0) throw new Error('[pricingFtlTariffMaster] step 3 route card anchor not found.')

  const ltlCommercialPanel = `          <div v-if="shipmentModeForApi === 'Ltl'" class="crystal-soft space-y-4 p-4 md:p-5">
            <div>
              <p class="font-black">Tarifario LTL a utilizar</p>
              <p class="mt-1 text-xs font-semibold text-[var(--dh-text-muted)]">Cliente final y NVOCC tienen matrices comerciales distintas. La selección se usa para resolver la tarifa de la ruta.</p>
            </div>
            <div class="grid gap-3 md:grid-cols-2">
              <button type="button" class="crystal-choice min-h-[96px] text-left" :class="landLtlCommercialProfile === 'FinalClient' ? 'crystal-choice--active' : ''" @click="landLtlCommercialProfile = 'FinalClient'">
                <p class="font-black">Cliente final</p>
                <p class="mt-1 text-xs font-semibold text-[var(--dh-text-muted)]">Tarifario comercial GCF para cliente final.</p>
              </button>
              <button type="button" class="crystal-choice min-h-[96px] text-left" :class="landLtlCommercialProfile === 'Nvocc' ? 'crystal-choice--active' : ''" @click="landLtlCommercialProfile = 'Nvocc'">
                <p class="font-black">NVOCC</p>
                <p class="mt-1 text-xs font-semibold text-[var(--dh-text-muted)]">Tarifario GCF NVOCC para consolidadores.</p>
              </button>
            </div>
            <div class="rounded-[22px] border border-[rgb(var(--dh-primary-rgb)/0.22)] bg-[rgb(var(--dh-primary-rgb)/0.05)] p-4">
              <p class="font-black">Carga consolidada terrestre</p>
              <p class="mt-1 text-xs font-semibold text-[var(--dh-text-muted)]">El flete LTL se calcula por CBM con el mínimo de la ruta. La referencia de peso volumétrico es 1 CBM = 333.33 kg.</p>
              <div class="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
                <DhInput v-model.number="form.cargoWeightKg" type="number" min="0" step="0.01" label="Peso total (kg)" />
                <DhInput v-model.number="form.cargoPallets" type="number" min="1" step="1" label="Bultos / tarimas" />
