...
function patchWizard(source: string) {
  ...
  code = replaceOne(
    ...
    `            <p class="crystal-description">Seleccione el tipo comercial y luego complete la ruta. Un SPOT se vuelve a cotizar al duplicarse; un TARIFARIO conserva su snapshot cuando un cliente lo acepta.</p>\n          </div>\n\n          <div v-if="sellerRequestMode" class="crystal-soft border border-[rgb(var(--dh-primary-rgb)/0.28)] p-4 md:p-5">\n            <div class="grid gap-4 md:grid-cols-[minmax(0,1fr)_minmax(280px,420px)] md:items-end">\n              <div>\n                <p class="text-sm font-black">Prioridad / tiempo máximo</p>\n                <p class="mt-1 text-xs font-semibold text-[var(--dh-text-muted)]">Verde: 24 horas · Amarillo: 48 horas · Rojo: 72 horas. Esta prioridad es independiente de si la solicitud será SPOT o TARIFARIO.</p>\n              </div>\n              <DhSelect v-model="rateRequestPriority" label="Prioridad" :options="rateRequestPriorityOptions" />\n            </div>\n          </div>\n\n          <div class="grid gap-3 md:grid-cols-2">`,
