...
export function pricingWizardEnhancements(): Plugin {
  return {
    ...
    transform(source, id) {
      ...
      const totalVolumeAnchor = '      totalVolumeCbm: 0,\n      cargoLines: form.cargoDescription'
      const totalVolumeReplacement = `      totalVolumeCbm: shipmentModeForApi.value === 'Lcl' ? Math.max(1, number(lclRequestedCbm.value)) : 0,\n      cargoLines: form.cargoDescription`
      code = replaceOne(code, totalVolumeAnchor, totalVolumeReplacement, 'LCL total volume persistence')
