...
function patchWizard(source: string) {
  ...
  // Completing a seller request sends the rate and closes the request atomically.
  if (code.includes("'/attach-rate'")) {
    code = code.replace("'/attach-rate'", "'/complete-rate'")
  }

  // Persist the real POD as the commercial destination while retaining POE/POD in request context.
  code = replaceOne(
    code,
...
function isEditableExwPickup(line: { name?: string | null }) {
  const incoterm = normalizeCatalogValue(selectedIncoterm.value ? displayValue(selectedIncoterm.value) : '')
  if (!incoterm.includes('exw')) return false
  const name = normalizeCatalogValue(String(line.name ?? ''))
  return name.includes('pickup') || name.includes('pick up') || name.includes('recoleccion')
}
...
watch(
  ...
  ([mode]) => {
    ...
    else fclExtraContainers.value = fclExtraContainers.value.filter((row) => row.containerTypeId !== form.equipmentId)
  },
)
...
`
  ...
  code = code.split(`:disabled="line.costDetailType === 'AgentCharge' || line.costType !== 'Variable'"`).join(
    `:disabled="!isEditableExwPickup(line) && (line.costDetailType === 'AgentCharge' || line.costType !== 'Variable')"`,
  )
  code = code.split(`:disabled="line.costDetailType === 'AgentCharge'"`).join(
    `:disabled="line.costDetailType === 'AgentCharge' && !isEditableExwPickup(line)"`,
  )
  ...
  // Terms follow the real charge lines plus the exact free days from the selected rate.
  ...
  // Seller request keeps the complete FCL distribution and explicit route context.
  ...
  code = replaceOne(
    ...
    'seller FCL/POD request restore',
  )
  ...
  code = replaceOne(
    ...
    'seller FCL/POD request context',
  )
...
export function pricingCommercialAutomation20260906Fixed(): Plugin {
  return {
    name: 'dhole-pricing-commercial-automation-20260906-fixed',
    enforce: 'pre',
    transform(source, id) {
