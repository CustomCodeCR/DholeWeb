...
function patchWizard(source: string) {
  ...
  const contextEquipmentAnchor = `<strong>{{ displayValue(selectedEquipment) || 'Pendiente' }}</strong>`
  const contextEquipmentReplacement = `<strong>{{ shipmentModeForApi === 'Lcl' ? 'LCL' : displayValue(selectedEquipment) || 'Pendiente' }}</strong>`
  code = replaceOne(code, contextEquipmentAnchor, contextEquipmentReplacement, 'LCL context equipment label')
