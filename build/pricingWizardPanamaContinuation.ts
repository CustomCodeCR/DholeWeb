...
function patchWizard(source: string) {
  ...
  code = code.slice(0, stepThreeReturnIndex)
    + `\n    if (panamaContinuationActive.value && !form.podId) return false`
  ...
  const continuationSelector = `            <div v-if="panamaContinuationActive" class="rounded-2xl border border-[rgb(var(--dh-primary-rgb)/0.20)] bg-[rgb(var(--dh-primary-rgb)/0.04)] p-4 md:p-5">\n              <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">\n                <div class="min-w-0">\n                  <div class="flex flex-wrap items-center gap-2">\n                    <p class="font-black">Continuación desde Panamá</p>\n                    <DhBadge variant="primary">Multimodal Via Panamá</DhBadge>\n                  </div>\n                  <p class="mt-1 max-w-2xl text-xs font-semibold leading-relaxed text-[var(--dh-text-muted)]">\n                    Marítimo-terrestre se utiliza por defecto con POD tipo SD. Active doble marítimo únicamente cuando la continuación desde Panamá sea por otro tramo marítimo.\n                  </p>\n                </div>\n\n                <button\n                  type="button"\n                  class="group flex w-full shrink-0 items-center gap-3 rounded-xl border px-3.5 py-3 text-left shadow-sm transition-all duration-200 lg:w-auto lg:min-w-[330px]"\n                  :class="panamaContinuationMode === 'DoubleMaritime'\n                    ? 'border-[rgb(var(--dh-primary-rgb)/0.55)] bg-[rgb(var(--dh-primary-rgb)/0.13)] shadow-[0_8px_24px_rgb(var(--dh-primary-rgb)/0.10)]'\n                    : 'border-[var(--dh-border)] bg-[var(--dh-card)] hover:-translate-y-0.5 hover:border-[rgb(var(--dh-primary-rgb)/0.38)] hover:bg-[rgb(var(--dh-primary-rgb)/0.07)] hover:shadow-md'"\n                  @click="selectPanamaContinuationMode(panamaContinuationMode === 'DoubleMaritime' ? 'MaritimeLand' : 'DoubleMaritime')"\n                >\n                  <span\n                    class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl transition-colors"\n                    :class="panamaContinuationMode === 'DoubleMaritime'\n                      ? 'bg-[rgb(var(--dh-primary-rgb)/0.18)] text-[var(--dh-primary)]'\n                      : 'bg-[rgb(var(--dh-primary-rgb)/0.09)] text-[var(--dh-primary)] group-hover:bg-[rgb(var(--dh-primary-rgb)/0.14)]'"\n                  >\n                    <Ship class="h-5 w-5" />\n                  </span>\n\n                  <span class="min-w-0 flex-1">\n                    <span class="block text-sm font-black">\n                      {{ panamaContinuationMode === 'DoubleMaritime' ? 'Doble marítimo activo' : 'Doble marítimo' }}\n                    </span>\n                    <span class="mt-0.5 block text-[11px] font-semibold leading-relaxed text-[var(--dh-text-muted)]">\n                      {{ panamaContinuationMode === 'DoubleMaritime'\n                        ? 'POD tipo CY · Clic para volver a marítimo-terrestre'\n                        : 'Cambiar la continuación a un segundo tramo marítimo' }}\n                    </span>\n                  </span>\n\n                  <span\n                    class="shrink-0 rounded-full px-2.5 py-1 text-[10px] font-black tracking-wide"\n                    :class="panamaContinuationMode === 'DoubleMaritime'\n                      ? 'bg-[var(--dh-primary)] text-white'\n                      : 'bg-[rgb(var(--dh-primary-rgb)/0.10)] text-[var(--dh-primary)]'"\n                  >\n                    {{ panamaContinuationMode === 'DoubleMaritime' ? 'ACTIVO · CY' : 'POD CY' }}\n                  </span>\n                </button>\n              </div>\n\n              <p v-if="!form.podId" class="mt-3 text-xs font-black text-amber-600 dark:text-amber-300">Seleccione el POD final para poder continuar.</p>\n            </div>\n\n${equipmentRowAnchor}`
