...
function patchWizard(source: string) {
  ...
  // Keep the form-owned selector and this compatibility layer synchronized.
  const continuationModeWatcher = `watch(\n  () => form.panamaContinuationMode,\n  (mode) => {\n    if (mode !== 'DoubleMaritime' && mode !== 'MaritimeLand') return\n    if (panamaContinuationMode.value === mode) return\n    applyPanamaContinuationMode(mode)\n    clearPanamaContinuationResolution(false)\n  },\n)`
  const continuationWatcherAnchor = `watch(\n  () => selectedIncotermCode.value,`
  if (!code.includes(continuationWatcherAnchor)) {
    throw new Error('[pricingWizardPanamaContinuation] Continuation mode watcher anchor not found.')
  }
  code = code.replace(
    continuationWatcherAnchor,
    `${continuationModeWatcher}\n\n${continuationWatcherAnchor}`,
  )
  ...
  // Pantalla 3: el selector visual único se genera en scripts/ensure-pricing-panama-continuation.mjs.
  // Este plugin mantiene solamente la compatibilidad de estado y terminales SD/CY.
