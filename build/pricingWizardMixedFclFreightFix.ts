...
function patchWizard(source: string) {
  ...
  const freightLineReplacement = [
    "  if (visible.has('international_freight')) {",
    "    if (shipmentModeForApi.value === 'Fcl' && fclContainerAllocations.value.length > 1) {",
    '      fclContainerAllocations.value.forEach((allocation) => {',
    '        lines.push({',
  ...
  code = replaceRequired(code, freightLineAnchor, freightLineReplacement, 'mixed FCL rate lines')
