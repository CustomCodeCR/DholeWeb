...
import_marker = "import { formatDate, formatMoney } from '@/modules/pricing/utils/pricingFormat'\n"
commercial_import = """import {
...
if commercial_import not in source:
    if import_marker not in source:
        raise SystemExit('Import marker not found')
    source = source.replace(import_marker, import_marker + commercial_import, 1)
...
"""
new_visible = """const visibleSections = computed<RateSection[]>(() => {
...
"""
if old_visible in source:
    source = source.replace(old_visible, new_visible, 1)
elif new_visible not in source:
    raise SystemExit('visibleSections marker not found')

old_amounts = """function serviceAmounts(service: CatalogItemSelectDto) {
...
"""
new_amounts = """function serviceAmounts(service: CatalogItemSelectDto) {
  const serviceValue = normalizeCatalogValue(displayValue(service))
  if (serviceValue.includes('seguro') && serviceValue.includes('carga') && form.cargoValue > 0) {
...
"""
if old_amounts in source:
    source = source.replace(old_amounts, new_amounts, 1)
elif new_amounts not in source:
    raise SystemExit('serviceAmounts marker not found')

operational_marker = """  effectiveServices.value
    .filter((service) => !normalizeCatalogValue(displayValue(service)).includes('transporte internacional'))
"""
operational_block = """  buildOperationalLines({
...
"""
if operational_block not in source:
    if operational_marker not in source:
        raise SystemExit('Operational insertion marker not found')
    source = source.replace(operational_marker, operational_block + operational_marker, 1)

old_service_head = """      const meta = metadata(service)
...
"""
new_service_head = """      const meta = metadata(service)
      const name = displayValue(service)
      ...
      const optional = detailType === 'Insurance' || Boolean(meta?.optional)
"""
if old_service_head in source:
    source = source.replace(old_service_head, new_service_head, 1)
elif new_service_head not in source:
    raise SystemExit('Service canonical marker not found')

old_service_name = """        key: `service:${service.id}`,
        section,
        name,
        costDetailType: detailType,
"""
new_service_name = """        key: `service:${service.id}`,
        section,
        name: lineName,
        costDetailType: detailType,
"""
if old_service_name in source:
    source = source.replace(old_service_name, new_service_name, 1)
elif new_service_name not in source:
    raise SystemExit('Service line name marker not found')

old_fallback = """      costAmount: Math.max(form.cargoValue * 0.20, 35),
      saleAmount: Math.max(form.cargoValue * 0.65, 95),
"""
new_fallback = """      costAmount: calculateCargoInsurance(form.cargoValue, form.freightCost).cost,
...
if old_fallback in source:
    ...
    raise SystemExit('Insurance fallback marker not found')

old_notes = """    notes: line.manual ? 'Cargo manual agregado desde el wizard de Pricing.' : null,
"""
new_notes = """    notes: line.costDetailType === 'Insurance'
      ? cargoInsuranceNote(form.cargoValue, form.freightCost)
      : line.manual
        ? 'Cargo manual agregado desde el wizard de Pricing.'
        : null,
"""
if old_notes in source:
    source = source.replace(old_notes, new_notes, 1)
elif new_notes not in source:
    raise SystemExit('Detail notes marker not found')

save_marker = """  try {
    saving.value = true
    const equipmentName = displayValue(equipment!)
"""
save_terms = """  const serviceCodes = new Set(
  ...
  try {
    ...
    const equipmentName = displayValue(equipment!)
"""
if save_terms not in source:
    if save_marker not in source:
        raise SystemExit('Save marker not found')
    source = source.replace(save_marker, save_terms, 1)

old_terms = """      includes: selectedServices.value.map(displayValue).join('\\n'),
      subjectTo: [
      ...
      excludes: null,
"""
new_terms = """      includes: includeTerms.join('\\n') || null,
      subjectTo: subjectTerms.join('\\n') || null,
      excludes: excludeTerms.join('\\n') || null,
"""
if old_terms in source:
    source = source.replace(old_terms, new_terms, 1)
elif new_terms not in source:
    raise SystemExit('Commercial payload marker not found')
