...
import { CalendarClock, FileImage, FileText, Image, LayoutDashboard, Megaphone, Menu, Newspaper, Plus, Search, Settings2, Sparkles, Users, X } from 'lucide-vue-next'
...
const iconMap: Record<string, Component> = { dashboard: LayoutDashboard, file: FileText, news: Newspaper, media: FileImage, image: Image, blocks: LayoutDashboard, menu: Menu, search: Search, link: Search, settings: Settings2, form: FileText, inbox: FileText, users: Users, calendar: CalendarClock, campaign: Megaphone, sparkles: Sparkles }
...
watch(() => route.query.section, (value) => { activeSection.value = isMarketingSection(value) ? value : 'dashboard' }, { immediate: true })
...
const contentSection = computed(() => {
  const definitions: Partial<Record<MarketingSectionKey, { type: ContentType; title: string; singular: string }>> = {
    'content-pages': { type: 'Page', title: 'Páginas', singular: 'Página' }, 'content-news': { type: 'News', title: 'Noticias', singular: 'Noticia' },
    'content-posts': { type: 'Post', title: 'Posts', singular: 'Post' }, 'content-videos': { type: 'Video', title: 'Videos', singular: 'Video' },
    'content-reusable': { type: 'ReusableBlock', title: 'Bloques reutilizables', singular: 'Bloque' }, 'design-banners': { type: 'Banner', title: 'Banners', singular: 'Banner' },
    'seo-pages': { type: 'Page', title: 'SEO de páginas', singular: 'Página' }, 'campaigns-landings': { type: 'Page', title: 'Landing Pages', singular: 'Landing page' },
  }
...
async function loadDashboard() {
  ...
  try {
    const [editor, leads, forms, meetings, campaigns] = await Promise.all([ContentService.getEditorDashboard(siteKey), MarketingService.getLeads(siteKey), MarketingService.getForms(siteKey), MarketingService.getMeetingRequests(), MarketingService.getCampaigns(siteKey)])
  ...
  } catch (error) { toastStore.backendWarning(error, 'No se pudo actualizar el resumen completo de Mercadeo.') }
  finally { dashboardLoading.value = false }
}
async function openSection(section: MarketingSectionKey) { activeSection.value = section; addMenuOpen.value = false; await router.replace({ query: { ...route.query, section } }) }
async function createContent(section: 'content-pages' | 'content-news' | 'design-banners') { addMenuOpen.value = false; await openSection(section); await nextTick(); createNonce.value += 1 }
...
<template>
  <section class="space-y-6">
    <DhPageHeader title="Mercadeo" :subtitle="`Administración del sitio · ${currentLabel}`" :icon="Megaphone">
      <template #actions>
        <div class="relative">
          <button class="primary-action" @click="addMenuOpen = !addMenuOpen"><Plus class="h-4 w-4" /> Agregar nuevo</button>
          <div v-if="addMenuOpen" class="create-menu">
            <div class="flex items-center justify-between border-b border-[var(--dh-border)] px-4 py-3"><strong class="text-sm">¿Qué desea crear?</strong><button class="rounded-lg p-1 opacity-55 hover:bg-black/5" @click="addMenuOpen = false"><X class="h-4 w-4" /></button></div>
      ...
      <aside class="dh-glass dh-liquid max-h-[calc(100vh-9rem)] overflow-y-auto rounded-[28px] p-2 xl:sticky xl:top-4">
        <nav class="space-y-4">
          <section v-for="(group, groupIndex) in MARKETING_GROUPS" :key="group.label ?? `root-${groupIndex}`">
            ...
            <div class="space-y-1"><button v-for="item in group.items" :key="item.key" class="nav-item" :class="{ 'nav-item-active': activeSection === item.key }" @click="openSection(item.key)"><component :is="iconMap[item.icon]" class="h-4 w-4 shrink-0" /><span>{{ item.label }}</span></button></div>
          </section>
        </nav>
      </aside>
      <main class="min-w-0">
        <div v-if="activeSection === 'dashboard'" class="space-y-5">
          <section class="dh-glass dh-liquid rounded-[30px] p-6">
            <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><p class="text-xs font-black uppercase tracking-[.18em] text-[var(--dh-primary)]">Dashboard</p><h2 class="mt-1 text-2xl font-black">Resumen de Mercadeo</h2><p class="mt-1 text-sm opacity-60">Contenido, captación, reuniones y campañas en un solo lugar.</p></div><span v-if="dashboardLoading" class="text-xs opacity-50">Actualizando…</span></div>
            <div class="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><button v-for="card in dashboardCards" :key="card.label" class="dashboard-card" @click="openSection(card.section)"><component :is="card.icon" class="h-5 w-5 text-[var(--dh-primary)]" /><strong class="mt-4 text-3xl">{{ card.value }}</strong><span class="mt-1 text-sm opacity-60">{{ card.label }}</span></button></div>
          </section>
          <section class="grid gap-5 lg:grid-cols-[1fr_.9fr]">
            <div class="dh-glass dh-liquid rounded-[30px] p-6"><h3 class="text-lg font-black">Crear contenido</h3><p class="mt-1 text-sm opacity-60">Accesos rápidos para las publicaciones más frecuentes.</p><div class="mt-5 grid gap-3 sm:grid-cols-3"><button class="quick-create" @click="createContent('content-pages')"><FileText class="h-6 w-6" /><span>Página</span></button><button class="quick-create" @click="createContent('content-news')"><Newspaper class="h-6 w-6" /><span>Noticia</span></button><button class="quick-create" @click="createContent('design-banners')"><Image class="h-6 w-6" /><span>Banner</span></button></div></div>
            <div class="dh-glass dh-liquid rounded-[30px] p-6"><h3 class="text-lg font-black">Flujo editorial</h3><div class="mt-5 space-y-3"><div class="status-row"><span>Drafts</span><strong>{{ dashboard.drafts }}</strong></div><div class="status-row"><span>Pendientes de aprobación</span><strong>{{ dashboard.pendingReview }}</strong></div><div class="status-row"><span>Programados</span><strong>{{ dashboard.scheduled }}</strong></div><div class="status-row"><span>Publicados</span><strong>{{ dashboard.published }}</strong></div></div></div>
          </section>
        </div>
        <MarketingAnimationTab v-else-if="activeSection === 'design-animations'" :site-key="siteKey" />
        <MarketingContentTab v-else-if="contentSection" :key="activeSection" :site-key="siteKey" :content-type="contentSection.type" :title="contentSection.title" :singular="contentSection.singular" :create-nonce="createNonce" />
        ...
        <section v-else-if="activeSection === 'seo-sitemap'" class="dh-glass dh-liquid rounded-[30px] p-6"><p class="text-xs font-black uppercase tracking-[.18em] text-[var(--dh-primary)]">SEO</p><h2 class="mt-1 text-2xl font-black">Sitemap</h2><p class="mt-2 max-w-3xl text-sm opacity-60">El sitemap se deriva de las rutas, el estado de publicación y la configuración SEO del contenido. Esta fase no crea una segunda fuente de datos ni un editor paralelo.</p></section>
        <MarketingResourceTab v-else-if="resourceSection" :key="activeSection" :kind="resourceSection.kind" :title="resourceSection.title" :subtitle="resourceSection.subtitle" :site-key="siteKey" />
      </main>
    </div>
  </section>
</template>
