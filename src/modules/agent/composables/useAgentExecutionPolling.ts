...
export function useAgentExecutionPolling(options: {
  ...
  function schedule() {
    ...
    if (disposed || document.hidden || !isAgentExecutionPollingStatus(options.getStatus())) return
  ...
  function sync() {
    ...
    if (isAgentExecutionPollingStatus(options.getStatus())) schedule()
    else stop()
  }
