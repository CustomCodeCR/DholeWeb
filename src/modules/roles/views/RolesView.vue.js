import { computed, onMounted, ref, watch } from 'vue';
import { Pencil, Shield, Trash2 } from 'lucide-vue-next';
import { useI18n } from 'vue-i18n';
import { DhBadge, DhButton } from '@/shared/components/atoms';
import { DhCrudToolbar, DhDataTable, DhPagination } from '@/shared/components/molecules';
import { DhPageHeader } from '@/shared/components/organisms';
import { RolesService } from '@/core/services/rolesService';
import { useDrawerStore } from '@/core/stores/drawerStore';
import { useModalStore } from '@/core/stores/modalStore';
import { useToastStore } from '@/core/stores/toastStore';
import { useAuthStore } from '@/core/stores/authStore';
import { AUTH_SCOPES } from '@/core/auth/scopes';
import RoleFormDrawer from '@/modules/roles/components/RoleFormDrawer.vue';
import RoleDetailDrawer from '@/modules/roles/components/RoleDetailDrawer.vue';
import DhConfirmDialog from '@/shared/components/molecules/DhConfirmDialog.vue';
import { useViewShortcuts } from '@/core/composables/useViewShortcuts';
const { t } = useI18n();
const drawerStore = useDrawerStore();
const modalStore = useModalStore();
const toastStore = useToastStore();
const authStore = useAuthStore();
const loading = ref(false);
const search = ref('');
const page = ref(1);
const pageSize = ref(10);
const total = ref(0);
const roles = ref([]);
const canCreate = computed(() => authStore.hasScope(AUTH_SCOPES.roles.create));
const canUpdate = computed(() => authStore.hasScope(AUTH_SCOPES.roles.update));
const canDelete = computed(() => authStore.hasScope(AUTH_SCOPES.roles.delete));
const showRowActions = computed(() => canUpdate.value || canDelete.value);
const columns = computed(() => {
    const base = [
        { key: 'name', label: t('common.name') },
        { key: 'description', label: t('common.description') },
        { key: 'isSystemRole', label: t('roles.systemRole'), align: 'center' },
        { key: 'isActive', label: t('common.status'), align: 'center' },
    ];
    if (showRowActions.value) {
        base.push({ key: 'actions', label: '', align: 'right' });
    }
    return base;
});
async function loadRoles() {
    try {
        loading.value = true;
        const result = await RolesService.browsePaged({ pageNumber: page.value, pageSize: pageSize.value, search: search.value || undefined });
        roles.value = result.items;
        total.value = result.totalCount ?? result.items.length;
    }
    catch (error) {
        toastStore.backendError(error, 'No se pudieron cargar los roles.');
    }
    finally {
        loading.value = false;
    }
}
function createRole() {
    if (!canCreate.value)
        return;
    drawerStore.open({ title: t('roles.new'), component: RoleFormDrawer, size: 'lg', props: { onSaved: loadRoles } });
}
function editRole(role) {
    if (!canUpdate.value)
        return;
    drawerStore.open({ title: t('common.edit'), component: RoleFormDrawer, size: 'lg', props: { role, onSaved: loadRoles } });
}
function detailRole(role) {
    drawerStore.open({ title: role.name, component: RoleDetailDrawer, size: 'xl', props: { role, onSaved: loadRoles } });
}
function confirmDelete(role) {
    if (!canDelete.value || role.isSystemRole)
        return;
    modalStore.open({
        title: t('common.delete'),
        component: DhConfirmDialog,
        size: 'md',
        props: {
            title: t('common.delete'),
            message: `¿Eliminar ${role.name}?`,
            confirmLabel: t('common.delete'),
            cancelLabel: t('common.cancel'),
            danger: true,
            onConfirm: async () => {
                await RolesService.delete(role.id);
                modalStore.close();
                toastStore.success('Rol eliminado');
                await loadRoles();
            },
            onCancel: () => modalStore.close(),
        },
    });
}
watch([page, pageSize], loadRoles);
useViewShortcuts({
    create: () => {
        if (canCreate.value)
            createRole();
    },
    save: loadRoles,
    refresh: loadRoles,
});
onMounted(loadRoles);
const __VLS_ctx = {
    ...{},
    ...{},
};
let __VLS_components;
let __VLS_intrinsics;
let __VLS_directives;
__VLS_asFunctionalElement1(__VLS_intrinsics.section, __VLS_intrinsics.section)({
    ...{ class: "space-y-6" },
});
/** @type {__VLS_StyleScopedClasses['space-y-6']} */ ;
let __VLS_0;
/** @ts-ignore @type { | typeof __VLS_components.DhPageHeader | typeof __VLS_components.DhPageHeader} */
DhPageHeader;
// @ts-ignore
const __VLS_1 = __VLS_asFunctionalComponent1(__VLS_0, new __VLS_0({
    title: (__VLS_ctx.t('roles.title')),
    subtitle: (__VLS_ctx.t('roles.subtitle')),
    icon: (__VLS_ctx.Shield),
}));
const __VLS_2 = __VLS_1({
    title: (__VLS_ctx.t('roles.title')),
    subtitle: (__VLS_ctx.t('roles.subtitle')),
    icon: (__VLS_ctx.Shield),
}, ...__VLS_functionalComponentArgsRest(__VLS_1));
const { default: __VLS_5 } = __VLS_3.slots;
if (__VLS_ctx.canCreate) {
    {
        const { actions: __VLS_6 } = __VLS_3.slots;
        let __VLS_7;
        /** @ts-ignore @type { | typeof __VLS_components.DhButton} */
        DhButton;
        // @ts-ignore
        const __VLS_8 = __VLS_asFunctionalComponent1(__VLS_7, new __VLS_7({
            ...{ 'onClick': {} },
            label: (__VLS_ctx.t('roles.new')),
        }));
        const __VLS_9 = __VLS_8({
            ...{ 'onClick': {} },
            label: (__VLS_ctx.t('roles.new')),
        }, ...__VLS_functionalComponentArgsRest(__VLS_8));
        let __VLS_12;
        const __VLS_13 = {
            /** @type {typeof __VLS_12.click} */
            onClick: (__VLS_ctx.createRole),
        };
        var __VLS_10;
        var __VLS_11;
        // @ts-ignore
        [t, t, t, Shield, canCreate, createRole,];
    }
}
// @ts-ignore
[];
var __VLS_3;
__VLS_asFunctionalElement1(__VLS_intrinsics.section, __VLS_intrinsics.section)({
    ...{ class: "dh-glass dh-liquid rounded-[32px] p-5" },
});
/** @type {__VLS_StyleScopedClasses['dh-glass']} */ ;
/** @type {__VLS_StyleScopedClasses['dh-liquid']} */ ;
/** @type {__VLS_StyleScopedClasses['rounded-[32px]']} */ ;
/** @type {__VLS_StyleScopedClasses['p-5']} */ ;
let __VLS_14;
/** @ts-ignore @type { | typeof __VLS_components.DhCrudToolbar} */
DhCrudToolbar;
// @ts-ignore
const __VLS_15 = __VLS_asFunctionalComponent1(__VLS_14, new __VLS_14({
    ...{ 'onCreate': {} },
    ...{ 'onRefresh': {} },
    ...{ 'onFilter': {} },
    ...{ 'onSearch': {} },
    search: (__VLS_ctx.search),
    title: (__VLS_ctx.t('roles.title')),
    createLabel: (__VLS_ctx.t('roles.new')),
    showCreate: (__VLS_ctx.canCreate),
}));
const __VLS_16 = __VLS_15({
    ...{ 'onCreate': {} },
    ...{ 'onRefresh': {} },
    ...{ 'onFilter': {} },
    ...{ 'onSearch': {} },
    search: (__VLS_ctx.search),
    title: (__VLS_ctx.t('roles.title')),
    createLabel: (__VLS_ctx.t('roles.new')),
    showCreate: (__VLS_ctx.canCreate),
}, ...__VLS_functionalComponentArgsRest(__VLS_15));
let __VLS_19;
const __VLS_20 = {
    /** @type {typeof __VLS_19.create} */
    onCreate: (__VLS_ctx.createRole),
};
const __VLS_21 = {
    /** @type {typeof __VLS_19.refresh} */
    onRefresh: (__VLS_ctx.loadRoles),
};
const __VLS_22 = {
    /** @type {typeof __VLS_19.filter} */
    onFilter: (__VLS_ctx.loadRoles),
};
const __VLS_23 = {
    /** @type {typeof __VLS_19.search} */
    onSearch: (__VLS_ctx.loadRoles),
};
var __VLS_17;
var __VLS_18;
__VLS_asFunctionalElement1(__VLS_intrinsics.div, __VLS_intrinsics.div)({
    ...{ class: "mt-5" },
});
/** @type {__VLS_StyleScopedClasses['mt-5']} */ ;
let __VLS_24;
/** @ts-ignore @type { | typeof __VLS_components.DhDataTable | typeof __VLS_components.DhDataTable} */
DhDataTable;
// @ts-ignore
const __VLS_25 = __VLS_asFunctionalComponent1(__VLS_24, new __VLS_24({
    ...{ 'onRowClick': {} },
    columns: (__VLS_ctx.columns),
    rows: (__VLS_ctx.roles),
    loading: (__VLS_ctx.loading),
    emptyText: (__VLS_ctx.t('roles.empty')),
}));
const __VLS_26 = __VLS_25({
    ...{ 'onRowClick': {} },
    columns: (__VLS_ctx.columns),
    rows: (__VLS_ctx.roles),
    loading: (__VLS_ctx.loading),
    emptyText: (__VLS_ctx.t('roles.empty')),
}, ...__VLS_functionalComponentArgsRest(__VLS_25));
let __VLS_29;
const __VLS_30 = {
    /** @type {typeof __VLS_29.rowClick} */
    onRowClick: (__VLS_ctx.detailRole),
};
const { default: __VLS_31 } = __VLS_27.slots;
{
    const { 'cell-isSystemRole': __VLS_32 } = __VLS_27.slots;
    const [{ value }] = __VLS_vSlot(__VLS_32);
    __VLS_asFunctionalElement1(__VLS_intrinsics.div, __VLS_intrinsics.div)({
        ...{ class: "flex justify-center" },
    });
    /** @type {__VLS_StyleScopedClasses['flex']} */ ;
    /** @type {__VLS_StyleScopedClasses['justify-center']} */ ;
    let __VLS_33;
    /** @ts-ignore @type { | typeof __VLS_components.DhBadge} */
    DhBadge;
    // @ts-ignore
    const __VLS_34 = __VLS_asFunctionalComponent1(__VLS_33, new __VLS_33({
        label: (value ? __VLS_ctx.t('common.yes') : __VLS_ctx.t('common.no')),
        variant: (value ? 'primary' : 'neutral'),
    }));
    const __VLS_35 = __VLS_34({
        label: (value ? __VLS_ctx.t('common.yes') : __VLS_ctx.t('common.no')),
        variant: (value ? 'primary' : 'neutral'),
    }, ...__VLS_functionalComponentArgsRest(__VLS_34));
    // @ts-ignore
    [t, t, t, t, t, canCreate, createRole, search, loadRoles, loadRoles, loadRoles, columns, roles, loading, detailRole,];
}
{
    const { 'cell-isActive': __VLS_38 } = __VLS_27.slots;
    const [{ value }] = __VLS_vSlot(__VLS_38);
    __VLS_asFunctionalElement1(__VLS_intrinsics.div, __VLS_intrinsics.div)({
        ...{ class: "flex justify-center" },
    });
    /** @type {__VLS_StyleScopedClasses['flex']} */ ;
    /** @type {__VLS_StyleScopedClasses['justify-center']} */ ;
    let __VLS_39;
    /** @ts-ignore @type { | typeof __VLS_components.DhBadge} */
    DhBadge;
    // @ts-ignore
    const __VLS_40 = __VLS_asFunctionalComponent1(__VLS_39, new __VLS_39({
        label: (value ? __VLS_ctx.t('common.active') : __VLS_ctx.t('common.inactive')),
        variant: (value ? 'success' : 'neutral'),
    }));
    const __VLS_41 = __VLS_40({
        label: (value ? __VLS_ctx.t('common.active') : __VLS_ctx.t('common.inactive')),
        variant: (value ? 'success' : 'neutral'),
    }, ...__VLS_functionalComponentArgsRest(__VLS_40));
    // @ts-ignore
    [t, t,];
}
if (__VLS_ctx.showRowActions) {
    {
        const { 'cell-actions': __VLS_44 } = __VLS_27.slots;
        const [{ row }] = __VLS_vSlot(__VLS_44);
        __VLS_asFunctionalElement1(__VLS_intrinsics.div, __VLS_intrinsics.div)({
            ...{ class: "flex justify-end gap-1" },
        });
        /** @type {__VLS_StyleScopedClasses['flex']} */ ;
        /** @type {__VLS_StyleScopedClasses['justify-end']} */ ;
        /** @type {__VLS_StyleScopedClasses['gap-1']} */ ;
        if (__VLS_ctx.canUpdate) {
            __VLS_asFunctionalElement1(__VLS_intrinsics.button, __VLS_intrinsics.button)({
                ...{ onClick: (...[$event]) => {
                        if (!(__VLS_ctx.showRowActions))
                            return;
                        if (!(__VLS_ctx.canUpdate))
                            return;
                        __VLS_ctx.editRole(row);
                        // @ts-ignore
                        [showRowActions, canUpdate, editRole,];
                    } },
                ...{ class: "rounded-2xl p-2 hover:bg-black/5 dark:hover:bg-white/10" },
                title: "Editar",
            });
            /** @type {__VLS_StyleScopedClasses['rounded-2xl']} */ ;
            /** @type {__VLS_StyleScopedClasses['p-2']} */ ;
            /** @type {__VLS_StyleScopedClasses['hover:bg-black/5']} */ ;
            /** @type {__VLS_StyleScopedClasses['dark:hover:bg-white/10']} */ ;
            let __VLS_45;
            /** @ts-ignore @type { | typeof __VLS_components.Pencil} */
            Pencil;
            // @ts-ignore
            const __VLS_46 = __VLS_asFunctionalComponent1(__VLS_45, new __VLS_45({
                ...{ class: "h-4 w-4" },
            }));
            const __VLS_47 = __VLS_46({
                ...{ class: "h-4 w-4" },
            }, ...__VLS_functionalComponentArgsRest(__VLS_46));
            /** @type {__VLS_StyleScopedClasses['h-4']} */ ;
            /** @type {__VLS_StyleScopedClasses['w-4']} */ ;
        }
        if (__VLS_ctx.canDelete && !row.isSystemRole) {
            __VLS_asFunctionalElement1(__VLS_intrinsics.button, __VLS_intrinsics.button)({
                ...{ onClick: (...[$event]) => {
                        if (!(__VLS_ctx.showRowActions))
                            return;
                        if (!(__VLS_ctx.canDelete && !row.isSystemRole))
                            return;
                        __VLS_ctx.confirmDelete(row);
                        // @ts-ignore
                        [canDelete, confirmDelete,];
                    } },
                ...{ class: "rounded-2xl p-2 text-red-500 hover:bg-red-500/10" },
                title: "Eliminar",
            });
            /** @type {__VLS_StyleScopedClasses['rounded-2xl']} */ ;
            /** @type {__VLS_StyleScopedClasses['p-2']} */ ;
            /** @type {__VLS_StyleScopedClasses['text-red-500']} */ ;
            /** @type {__VLS_StyleScopedClasses['hover:bg-red-500/10']} */ ;
            let __VLS_50;
            /** @ts-ignore @type { | typeof __VLS_components.Trash2} */
            Trash2;
            // @ts-ignore
            const __VLS_51 = __VLS_asFunctionalComponent1(__VLS_50, new __VLS_50({
                ...{ class: "h-4 w-4" },
            }));
            const __VLS_52 = __VLS_51({
                ...{ class: "h-4 w-4" },
            }, ...__VLS_functionalComponentArgsRest(__VLS_51));
            /** @type {__VLS_StyleScopedClasses['h-4']} */ ;
            /** @type {__VLS_StyleScopedClasses['w-4']} */ ;
        }
        // @ts-ignore
        [];
    }
}
// @ts-ignore
[];
var __VLS_27;
var __VLS_28;
__VLS_asFunctionalElement1(__VLS_intrinsics.div, __VLS_intrinsics.div)({
    ...{ class: "mt-5" },
});
/** @type {__VLS_StyleScopedClasses['mt-5']} */ ;
let __VLS_55;
/** @ts-ignore @type { | typeof __VLS_components.DhPagination} */
DhPagination;
// @ts-ignore
const __VLS_56 = __VLS_asFunctionalComponent1(__VLS_55, new __VLS_55({
    page: (__VLS_ctx.page),
    pageSize: (__VLS_ctx.pageSize),
    total: (__VLS_ctx.total),
}));
const __VLS_57 = __VLS_56({
    page: (__VLS_ctx.page),
    pageSize: (__VLS_ctx.pageSize),
    total: (__VLS_ctx.total),
}, ...__VLS_functionalComponentArgsRest(__VLS_56));
// @ts-ignore
[page, pageSize, total,];
const __VLS_export = (await import('vue')).defineComponent({});
export default {};
