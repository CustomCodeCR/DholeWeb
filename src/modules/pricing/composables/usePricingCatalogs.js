...
async function loadFirstAvailable(slugs) {
    for (const slug of slugs) {
        try {
            const items = await CatalogItemsService.getByGroupSlug(slug);
            if (items.length > 0)
                return items.filter((item) => item.isActive !== false).map(mapItem);
        }
        catch {
...
async function loadAll(force = false) {
    ...
    loading.value = true;
    activeLoad = (async () => {
        const [agentRows, carrierRows, currencyRows, polRows, poeRows, podRows, containerRows, profileRows,] = await Promise.all([
...
function findByCode(items, value) {
    const target = normalize(value ?? '');
    if (!target)
        return undefined;
    return items.find((item) => [item.code, item.name, item.slug, item.value].some((candidate) => normalize(candidate) === target));
}
function compact(value) {
    return normalize(value).replace(/[^a-z0-9]/g, '');
}
function findBestMatch(items, id, ...values) {
    ...
    for (const item of items) {
        ...
        for (const candidate of candidates) {
            ...
            if (!normalizedCandidate)
                continue;
            let score = 0;
            if (itemValues.includes(normalizedCandidate))
                score = 1000;
            else if (compactCandidate && compactItemValues.includes(compactCandidate))
                score = 950;
            else if (normalizedCandidate.length >= 3 &&
                itemValues.some((value) => value.includes(normalizedCandidate) || normalizedCandidate.includes(value))) {
                score = 700 + Math.min(normalizedCandidate.length, 100);
            }
            if (!best || score > best.score)
                best = score > 0 ? { item, score } : best;
        }
    }
    return best?.item;
}
export function usePricingCatalogs() {
    function resolveRateLabels(rate) {
        const label = (items, id, fallback) => items.find((item) => item.id === id)?.name || fallback || '—';
        return {
            ...rate,
    ...
    function resolveCostLabels(cost) {
