...
const { activeTab, importSearch, importStatus, rateSearch, rateActive, costSearch, uploadProfileCode } = storeToRefs(pricingStore)
...
watch([importSearch, importStatus], () => {
  importPage.value = 1
})

watch([rateSearch, rateActive, selectedRatePort], () => {
  ratePage.value = 1
})
...
watch([activeTab, importSearch, importStatus, rateSearch, rateActive, costSearch, uploadProfileCode], () => {
...
function rateMatrixRows(rate: RateHeaderDto) {
  ...
  const costRows = (rate.costDetails ?? []).map((detail, index) => ({
    ...
    sale: rowAmount(detail.saleAmount ?? detail.amount),
    currency: detail.currencyCodeSnapshot || rate.currencyCodeSnapshot || 'USD',
    locked: detail.isFixed && !detail.isManual,
  }))
...
function resetCostForm(cost?: CostDto) {
  ...
  costForm.carrier = cost?.carrierId ?? findItemByText('carriers', cost?.carrierCodeSnapshot)?.value ?? ''
  costForm.port = cost?.portId ?? findItemByText('ports', cost?.portCodeSnapshot)?.value ?? ''
  costForm.portRole = cost?.portRole ?? 'Pod'
  costForm.currency = cost?.currencyId ?? findItemByText('currencies', cost?.currencyCodeSnapshot)?.value ?? ''
...
async function loadCosts() {
  const query: BrowseCostsQuery = { pageNumber: 1, pageSize: 50, search: nullableText(costSearch.value) }
  const result = await PricingService.browseCosts(query)
  costs.value = result.items
}
...
async function loadImports() {
  ...
  const result = await PricingService.browseImportFclRates(query)
...
async function loadRates() {
  ...
  const result = await PricingService.browseRateHeaders(query)
...
async function refreshAll() {
  loading.value = true
  try {
    await Promise.all([loadCatalogSelects(), loadCosts(), loadImports(), loadRates()])
  } catch (error) {
...
function applyCostTemplate(value: string | number) {
  ...
  rateCostForm.currency = findItemByText('currencies', cost.currencyCodeSnapshot)?.value ?? ''
...
function canChangeRateCostDetail(row: Record<string, unknown>): boolean {
  return !Boolean(row.isFixed) || Boolean(row.isManual)
}
...
function openEditRateCostDetail(row: Record<string, unknown>) {
  ...
  rateCostEditForm.currency = String(row.currencyId ?? findItemByText('currencies', String(row.currencyCodeSnapshot ?? selectedRate.value?.currencyCodeSnapshot ?? ''))?.value ?? '')
...
async function removeVariableRateCostDetail(row: Record<string, unknown>) {
      ...
      <div class="mt-5 space-y-4">
        <article class="rounded-[24px] border border-[var(--dh-border)] bg-[var(--dh-card)] p-4">
          ...
          <div v-if="automaticCostPortGroups.length" class="grid gap-4 xl:grid-cols-2">
            ...
            >
              <header class="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--dh-border)] p-4">
                <div>
                ...
                <div class="grid grid-cols-2 gap-2 text-right">
                  <div>
                    <p class="text-[10px] font-black uppercase tracking-[0.12em] text-[var(--dh-text-muted)]">{{ t('pricing.matrix.cost') }}</p>
                    <p class="font-black text-[var(--dh-text)]">{{ money(group.totalCost, group.rows[0]?.currencyCodeSnapshot || 'USD') }}</p>
                  </div>
                  <div>
                    <p class="text-[10px] font-black uppercase tracking-[0.12em] text-[var(--dh-text-muted)]">{{ t('pricing.matrix.sale') }}</p>
                    <p class="font-black text-[var(--dh-text)]">{{ money(group.totalSale, group.rows[0]?.currencyCodeSnapshot || 'USD') }}</p>
                  </div>
                </div>
              </header>
        ...
        <section class="grid gap-5 xl:grid-cols-2">
          ...
          <article class="space-y-3">
            ...
            <DhDataTable :columns="costDetailColumns" :rows="selectedRateCostRows" :empty-text="t('pricing.rates.noCostDetails')">
              ...
              <template #cell-__actions="{ row }">
                <div v-if="canChangeRateCostDetail(row)" class="flex justify-end gap-2">
                  <DhButton
                    size="sm"
                    variant="secondary"
                    :icon="Pencil"
                  ...
                  <DhButton
                    ...
                    @click.stop="removeVariableRateCostDetail(row)"
                  />
                </div>
                <DhBadge v-else variant="neutral" :label="t('pricing.rates.fixedLocked')" />
              </template>
            </DhDataTable>
          </article>
        </section>
