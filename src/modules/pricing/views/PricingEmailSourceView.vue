...
<template>
  <section class="min-w-0 space-y-4 p-0.5 sm:space-y-5 sm:p-1">
    ...
    <template v-else-if="source && message">
      <article class="min-w-0 rounded-[24px] border border-[var(--dh-border)] bg-[var(--dh-card)] p-3 sm:rounded-[28px] sm:p-5">
        <div class="flex flex-wrap items-start justify-between gap-3">
          <div class="min-w-0">
            <p class="text-xs font-black uppercase tracking-[0.14em] text-[var(--dh-primary)]">Asunto original</p>
            <h2 class="mt-2 break-words text-lg font-black text-[var(--dh-text)] sm:text-xl">{{ message.subject }}</h2>
        ...
        <div class="mt-5 max-h-[60vh] overflow-auto whitespace-pre-wrap break-words rounded-[20px] bg-black/[0.035] p-3 text-xs font-medium leading-5 sm:p-4 sm:text-sm sm:leading-6 text-[var(--dh-text)] dark:bg-white/[0.05]">{{ bodyText }}</div>
      </article>
