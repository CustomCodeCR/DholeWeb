...
export const useShortcutStore = defineStore('shortcuts', {
    ...
    getters: {
        byKeys: (state) => (keys) => state.shortcuts.find((x) => normalizeKeys(x.keys) === normalizeKeys(keys)),
        byAction: (state) => (action) => state.shortcuts.find((x) => x.action === action),
    },
    actions: {
        save() {
        ...
        updateShortcut(action, keys) {
            ...
            if (!normalized || MODIFIER_KEYS.has(normalized))
                return;
