...
export class ApiError extends Error {
    ...
    code;
    data;
    constructor(message, endpoint, method, status, code, data) {
        super(message);
        this.name = 'ApiError';
...
export class NetworkError extends ApiError {
    constructor(endpoint, method, originalError) {
...
export class ClientError extends ApiError {
    constructor(endpoint, method, status, message, data, code) {
...
function firstMessageFromArray(values) {
...
function extractValidationErrors(errors) {
    ...
    if (Array.isArray(errors))
        return firstMessageFromArray(errors);
    if (isObject(errors)) {
        const messages = [];
...
export function extractBackendCode(data) {
    let current = data;
...
export function extractBackendMessage(data) {
    ...
    for (let depth = 0; depth < 5; depth += 1) {
        ...
        if (Array.isArray(current))
            return firstMessageFromArray(current);
        if (!isObject(current))
            return null;
        const directMessage = clean(current.message) ??
        ...
        if (typeof errorValue === 'string')
            return clean(errorValue);
        if (Array.isArray(errorValue))
            return firstMessageFromArray(errorValue);
...
export function getApiErrorMessage(error, fallback = 'No se pudo completar la acción.') {
 ...
 */
export async function handleApiResponse(response, endpoint, method) {
    if (response.ok) {
        return;
    }
