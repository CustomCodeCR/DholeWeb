...
type ViewShortcutHandler = () => void | Promise<void>

export interface ViewShortcutOptions {
  create?: ViewShortcutHandler
  save?: ViewShortcutHandler
  refresh?: ViewShortcutHandler
  autoRefresh?: boolean
  debounceMs?: number
}

export function useViewShortcuts(options: ViewShortcutOptions) {
  ...
  async function run(handler?: ViewShortcutHandler) {
    if (!handler) return
    await handler()
  }
