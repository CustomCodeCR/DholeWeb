...
const screen08Marker = 'data-screen08-transit-validity'
if (!source.includes(screen08Marker)) {
  replaceRequired(
    ...
    `                <span>Margen <strong class="block">{{ totalMarginPercentage.toFixed(2) }}%</strong></span>
                <span>Operación <strong class="block">{{ direction }}</strong></span>
                <span data-screen08-transit-validity>Días de tránsito <strong class="block">{{ form.transitDays > 0 ? String(form.transitDays) + ' días' : '—' }}</strong></span>
                <span>Vigencia hasta <strong class="block">{{ formatDate(form.validTo) }}</strong></span>`,
    'días de tránsito y vigencia de Pantalla 08',
  )
} else {
  console.log('Pricing screen 08 transit/validity: already present')
}
...
console.log('Pricing wizard runtime patches: OK')
