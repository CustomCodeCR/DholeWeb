...
source = replaceOnce(
  source,
  `  destinationId: '',\n  podId: '',\n  equipmentSize: '',`,
  `  destinationId: '',\n  podId: '',\n  panamaContinuationMode: 'MaritimeLand' as PanamaContinuationMode,\n  selectedContinuationImportRateId: '',\n  equipmentSize: '',`,
...
source = replaceOnce(
  source,
  `    destinationId: '',\n    podId: '',\n    equipmentSize: '',`,
  `    destinationId: '',\n    podId: '',\n    panamaContinuationMode: 'MaritimeLand',\n    selectedContinuationImportRateId: '',\n    equipmentSize: '',`,
...
source = replaceOnce(
  ...
  'tipo de terminal POD dinámico Panamá',
)

source = replaceOnce(
  source,
  `            </div>\n\n            <!-- Fila 3: tamaño, tipo y cantidad del equipo. -->`,
  `            </div>\n\n            <div v-if="isPanamaMultimodal" class="rounded-2xl border border-[rgb(var(--dh-primary-rgb)/0.20)] bg-[rgb(var(--dh-primary-rgb)/0.04)] p-3.5 md:p-4">\n              <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">\n                <div class="min-w-0">\n                  <div class="flex flex-wrap items-center gap-2">\n                    <p class="font-black">Continuación desde Panamá</p>\n                    <DhBadge variant="primary">Marítimo-terrestre por defecto</DhBadge>\n                  </div>\n                  <p class="mt-1 max-w-2xl text-xs font-semibold leading-relaxed text-[var(--dh-text-muted)]">Use doble marítimo únicamente cuando desde Panamá la carga continúe en otro tramo marítimo. Al activarlo, el POD cambia a terminal tipo CY.</p>\n                </div>\n\n                <button\n                  type="button"\n                  class="group flex w-full shrink-0 items-center gap-3 rounded-xl border px-3.5 py-2.5 text-left shadow-sm transition-all duration-200 lg:w-auto lg:min-w-[310px]"\n                  :class="form.panamaContinuationMode === 'DoubleMaritime' ? 'border-[rgb(var(--dh-primary-rgb)/0.55)] bg-[rgb(var(--dh-primary-rgb)/0.13)] shadow-[0_8px_24px_rgb(var(--dh-primary-rgb)/0.10)]' : 'border-[var(--dh-border)] bg-[var(--dh-card)] hover:-translate-y-0.5 hover:border-[rgb(var(--dh-primary-rgb)/0.38)] hover:bg-[rgb(var(--dh-primary-rgb)/0.07)] hover:shadow-md'"\n                  @click="form.panamaContinuationMode = form.panamaContinuationMode === 'DoubleMaritime' ? 'MaritimeLand' : 'DoubleMaritime'"\n                >\n                  <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[rgb(var(--dh-primary-rgb)/0.10)] text-[var(--dh-primary)] transition-colors group-hover:bg-[rgb(var(--dh-primary-rgb)/0.14)]">\n                    <Ship class="h-4 w-4" />\n                  </span>\n                  <span class="min-w-0 flex-1">\n                    <span class="block text-sm font-black">{{ form.panamaContinuationMode === 'DoubleMaritime' ? 'Doble marítimo activo' : 'Doble marítimo' }}</span>\n                    <span class="mt-0.5 block text-[11px] font-semibold leading-relaxed text-[var(--dh-text-muted)]">{{ form.panamaContinuationMode === 'DoubleMaritime' ? 'Clic para volver al tramo terrestre' : 'Usar un segundo tramo marítimo' }}</span>\n                  </span>\n                  <span class="shrink-0 rounded-full px-2.5 py-1 text-[10px] font-black tracking-wide" :class="form.panamaContinuationMode === 'DoubleMaritime' ? 'bg-[var(--dh-primary)] text-white' : 'bg-[rgb(var(--dh-primary-rgb)/0.10)] text-[var(--dh-primary)]'">{{ form.panamaContinuationMode === 'DoubleMaritime' ? 'ACTIVO · CY' : 'POD CY' }}</span>\n                </button>\n              </div>\n              <p v-if="!form.podId" class="mt-3 text-xs font-bold text-amber-600">Seleccione el POD final para definir el segundo tramo.</p>\n            </div>\n\n            <!-- Fila 3: tamaño, tipo y cantidad del equipo. -->`,
